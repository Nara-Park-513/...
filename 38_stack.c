...
//스택에 저장할 수 있는 최대 원소 개수는 100개 입니다

typedef struct {//스택 구조체는 아래에 두 멤버를 포함
...
void initStack(Stack *s){
    s -> top = -1; //초기값이 -1로 설정되어 스택이 비어있음을 의미 lambda
}
...
int pop(Stack *s){
    if (isEmpty(s)){//스택이 비어 있으면 underflow메세지 출력
        ...
        return -1;//게시판 일 가정할 때 글쓰기 성공1, 실패0 -1네트워크 오류
    }
    return s -> data[(s -> top)--];
...
//스택에서 top값보기 Peek
int peek(Stack *s){//
    if(isEmpty(s)){//스택이 비어있으면 경고 메세지 출력
